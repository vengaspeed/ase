...
public class Order {
	...
	/**
	 * Set up the order details. The id is assigned according to the last id
	 * assigned and this one is updated.
	 * 
	 * @param item
	 *            The menu item ordered.
	 * @param quantity
	 ...
	 */
	public Order(final MenuItem item, final int quantity, final int tableId)
			throws ImpossibleQuantityException {
		this.item = item;
	...
	public double getDiscount() {
		return (double) Math.round(quantity * item.getPrice() * discount[1]
				* 100) / 100;
	}
